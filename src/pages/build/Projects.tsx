...
import { projectCards, demoWidgets } from '../../data/siteContent.ts'
...
export function ProjectsPage() {
  ...
  return (
    <div ref={ref} className="container space-y-16">
      <section className="space-y-12">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {projectCards.map((project) => (
            <article
              key={project.title}
              className="group flex flex-col rounded-3xl border border-neutral-900/10 bg-white p-6 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-brand-primary/30 dark:border-white/5 dark:bg-neutral-900/70 dark:hover:border-brand-primary/40"
              data-animate
            >
              ...
              <Link to="/contact" className="mt-auto inline-flex items-center gap-2 pt-6 text-sm font-semibold text-brand-primary transition-all group-hover:gap-3">
      ...
      <section className="space-y-12">
        <SectionHeading
          eyebrow="Demos"
          title="Test live demos of our enterprise systems."
          description="Interactive demonstrations — adjust variables and watch metrics update in real-time."
        />
          ...
          {demoWidgets.map((widget) => (
            <article key={widget.title} className="group rounded-3xl border border-neutral-900/10 bg-white/90 p-6 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-brand-primary/30 dark:border-white/10 dark:bg-neutral-900/80 dark:hover:border-brand-primary/40">
              ...
              <div className="mt-6 flex items-center gap-3 rounded-2xl border border-dashed border-brand-primary/30 bg-brand-primary/5 p-4">
                <div className="h-10 w-10 rounded-full bg-brand-primary/20 transition-colors duration-300 group-hover:bg-brand-primary/30"></div>
                <div>
